<template>
  <div class="tab-cont">
    <form @submit.prevent="onSubmit" onkeydown="if(event.keyCode==13){return false;}">
      <router-link :to="{name:'addurl',params:{ url:'url1' }}">
        <div class="detail-item">
          <span>添加链接</span>
          <div class="right">
            <input :value="$store.state.url1" disabled="true" placeholder="请添加所需要转发的链接">
            <img class="tz" src="../../assets/img/ic_405@2x.png">
          </div>
        </div>
      </router-link>
      <router-link :to="{name:'addimage',params:{ img:'imgArr1' }}">
        <div class="detail-item">
          <span>添加图片</span>
          <div class="right">
            <input
      ...
      <div class="detail-item">
        <span>链接标题</span>
        <input name="name" v-model="fromData.urlTitle" placeholder="请填写链接标题">
      </div>
      <div class="detail-item">
        <span>链接文字</span>
        <input name="name" v-model="fromData.urlCont" placeholder="请填写链接文字">
      </div>
      <div class="detail-item">
        <span>商户名字</span>
      ...
      <div class="detail-box">
        <div class="detail-item">
          <span>分享任务</span>
          <mt-switch v-model="fromData.shareSwitch"></mt-switch>
        </div>
        <div v-if="fromData.shareSwitch">
          <div class="detail-item">
            <span>广告分享个数</span>
            <input
              type="number"
    ...
    <mt-popup v-model="popupVisible" class="xy-pop" popup-transition="popup-fade">
      <div class="popup">
...
export default {
  ...
  methods: {
    ...
    onSubmit(e) {
      ...
      if (this.fromData.shareNum < 500 || this.fromData.ydNum < 500) {
        this.$toast("任务个数不能小于500");
        return;
      }
      this.fromData.shareSum = this.shareSum;
      this.fromData.ydSum = this.ydSum;
      // this.fromData.url = this.$global.addUrl;
      ...
      this.$store.commit("clearFrom", ["url1"]);
  ...
  mounted() {
    ...
    // this.fromData = this.$global.from1 || this.fromData;
  },
  beforeRouteLeave(to, from, next) {
    ...
    console.log(to);
    // this.$global.from1 = this.fromData;
